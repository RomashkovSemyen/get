...
class MCP4725:
    ...
    def set_number(self, number):
        ...
        if self.verbose:
            print(f"Число: {number}, отправленные по IC2 данные: [0x{(self.address << 1):02X}, 0x{first_byte:02X}, 0x{second_byte:02X}]\n")
    def set_voltage(self,voltage):
        if not(0 <= voltage <= self.dynamic_range):
            print("Напряжение выходит за достпуные границы!")
            return
